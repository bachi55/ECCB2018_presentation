...
%     \item Widely used analysis workflow: \alert<2>{Liquid chromatography (LC)} combined with \alert<3>{tandem mass spectrometry (MS/MS)}
    ...
    \item Molecular structure is not measured, but inference from the MS/MS spectrum
\end{itemize}
% \end{small}
\vspace{0.25cm}
...
\begin{figure}[c]
    \centering
    \includegraphics[width=\textwidth]{images/lcms_spectrum_complex_small_only_iokr_with_candidates.pdf}
\end{figure}
\vspace{-0.15cm}
\begin{small}
...
\vspace{-0.15cm}
\begin{enumerate}
    \item For each MS/MS spectrum $x_i$ define a set of \emph{molecular candidate structures} $\{m_{i,1},m_{i,2},\ldots\}$.\hfill\ex{using the molecular mass}
    \item Assign a ``MS/MS matching score'' $y_{i,j}$ to each candidate.\\\hfill\ex{Input-Ouput-Kernel-Regression\cite{Brouard2016}}
    \item \alert{Highest scoring candidate} $m_{i,j}$ is the identification for spectrum $x_i$.
\end{enumerate}
\end{block}
\end{small}
}
