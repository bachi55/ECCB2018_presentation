...
\begin{itemize}
    ...
    \item[$\circ$] remaning 222 RTs are used for RankSVM training ($\sys_{Impact}$)
    \item Additionally we use the 5 datasets $\hat{S}$ of the previous experiments for RankSVM training
\end{itemize}
\end{block}
\begin{block}{Evaluation measure and protocol}
\begin{itemize}
    \item randomly sample 1000 times 80 (MS/MS, RT)-tuples
    \item We construct the graph containing the candidates run the shortest path algorithm.
